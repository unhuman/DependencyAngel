...
public class PomManipulator {
    ...
    public void addForcedDependencyNode(Dependency dependency) {
        addComment(dependenciesNode, COMMENT_DEPENDENCY_ANGEL_START);
        addForcedDependencyNode(dependency.getGroupId(), dependency.getArtifactId(), dependency.getType(),
                dependency.getVersion(), dependency.getScope(), dependency.getClassifier(), dependency.getExclusions());
        addComment(dependenciesNode, COMMENT_DEPENDENCY_ANGEL_START);
    }

    public void addComment(Node parentNode, String comment) {
        addLastChild(parentNode, document.createComment(comment));
    }
    ...
    public void addForcedDependencyNode(String groupId, String artifactId, String type, Version version, String scope,
        ...
        addComment(dependenciesNode, COMMENT_DEPENDENCY_ANGEL_END);
    }
    ...
    protected String storeVersionInProperties(String groupId, String artifactId, String version,
        ...
        if (versionElements.getLength() == 1) {
            ...
            if (needAngelTracking) {
                addComment(propertiesNode, COMMENT_DEPENDENCY_ANGEL_START);
                storableAngelConfigData.addManagedVersion(key);
            }
            addLastChild(propertiesNode, versionProperty);

            if (needAngelTracking) {
                addComment(propertiesNode, COMMENT_DEPENDENCY_ANGEL_END);
            }
        }
